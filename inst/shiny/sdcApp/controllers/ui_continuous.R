...
output$ui_topbotcoding_num <- renderUI({
  ...
  output$ui_topbot_params_num <- renderUI({
    txt_tooltip1 <- translate("In case of top, all values above the threshold are replaced, in the case of bottom, all values below the threshold are replaced.")
    txt_tooltip2 <- translate("All values below (bottom) or above (top) this threshold are replaced with the replacement value.")
    txt_tooltip3 <- translate("The replacement value is the value that replaces all the values below (bottom) or above (top) the specified threshold. Often the replacement value is the same as the threshold value.")
    sel_var <- selectInput("sel_topbot_var_num", choices=numVars(), selected=obj$inp_sel_topbot_var_num, multiple=FALSE, label=h5(translate("Select variable")), width="75%")
    sel_kind <- radioButtons("sel_topbot_kind_num", choices=c("top","bottom"),
      label=h5(translate("Apply top/bottom coding?"), tipify(icon("question"), title=txt_tooltip1, placement="top")), inline=TRUE)
    txt_val <- textInput("num_topbot_val_num", label=h5(translate("Threshold value"), tipify(icon("question"), title=txt_tooltip2, placement="top")),
      placeholder=translate("Please enter a number"), width="75%")
    txt_replace <- textInput("num_topbot_replacement_num", label=h5(translate("Replacement value"), tipify(icon("question"), title=txt_tooltip3, placement="top")),
      placeholder=translate("Please enter a number"), width="75%")
  ...
  output$ui_topbot_btn_num <- renderUI({
    ...
    if (is.numeric(num1) & is.numeric(num2)) {
      ...
      return(fluidRow(
        column(12, p(code(n),translate("(out of"),code(N),translate(") values will be replaced! This equals"),code(p),translate("percent of the data.")), align="center"),
        column(12, myActionButton("btn_topbotcoding_num",label=translate("Apply Top/Bottom-Coding"), "primary"), align="center")
      ))
    } else {
      return(NULL)
    }
  ...
  helptxt <- translate("Here you can recode all values in a variable below (bottom coding) or above (top coding) a certain threshold. These values are replaced")
  helptxt <- paste(helptxt, translate("with the specified replacement value. The boxplot below shows the distribution of the data before top/bottom coding. "))
  helptxt <- paste(helptxt, translate("The bottom of the box is the 25th percentile and the top of the box the 75th percentile. The bar in the boxplot is the median. "))
  helptxt <- paste(helptxt, translate("The length of the whiskers is 1.5 times the interquartile range (IQR), unless the smallest/largest obeservation is closer to the box. Any value below/above the whiskers is indicated as outlier."))
  out <- fluidRow(
    column(12, h4(translate("Apply top/bottom coding"), align="center")),
...
output$ui_microaggregation <- renderUI({
  ...
  output$ui_microagg_clusterbased <- renderUI({
    ...
    txt_clnrcl <- translate("Specifies the number of clusters generated by the clustering algorithm")
    ...
    rb_transf <- radioButtons("rb_microagg_transf",
      label=h5(translate("Transformation"), tipify(icon("question"), title=txt_cltransf, placement="top")),
    ...
    sl_nc <- isolate(sliderInput("sl_microagg_nc",
      label=h5(translate("Number of clusters"), tipify(icon("question"), title=txt_clnrcl, placement="top")),
  ...
  output$ui_microagg_strata <- renderUI({
    txt_tooltip <- translate("By default microaggregation is applied on the complete dataset. To apply the algorithm within strata, select a variable for stratification. The algorithm is then applied within the strata defined by the factor levels of that variable.")
    selectInput("sel_microagg_strata",
      label=h5(translate("Apply microaggregation in groups (stratification)?"), tipify(icon("question"), title=txt_tooltip, placement="top")),
  ...
  output$ui_microagg_use_cluster <- renderUI({
    txt_tooltip <- translate("Cluster-based methods consists of two steps: first creating cluster and subsequently applying microaggregation methods within clusters. Several clustering methods can be chosen as well as parameters for these methods.")
    txt_tooltip <- paste0(txt_tooltip, translate("Clustering observations before applying microaggregation might be useful since the selected microaggregation algorithm is applied within clusters with a higher degree of homogeneity possibly resulting in lower levels of information loss."))
    radioButtons("rb_microagg_cluster",
      label=h5(translate("Use a cluster-based method?"), tipify(icon("question"), title=txt_tooltip, placement="bottom")),
  ...
  output$ui_microagg_method <- renderUI({
    ...
    if (input$rb_microagg_cluster=="No") {
      sel_method <- selectInput("sel_microagg_method",
        label=h5(translate("Select the method"), tipify(icon("question"), title=txt, placement="bottom")),
        choices=choices_aggmethods(), selected=input$sel_microagg_method, width="100%")
    } else {
      sel_method <- selectInput("sel_microagg_method",
        label=h5(translate("Select the method"), tipify(icon("question"), title=txtClus, placement="bottom")),
        choices=choices_aggmethods(), selected=input$sel_microagg_method, width="100%")
    }
  })
  output$ui_microagg_vars <- renderUI({
    txt_tooltip <- translate("Select numeric key variables for microaggregation. If no variables are selected, microaggregation is applied to all numeric key variables. If microaggregation is applied simultaneously to several variables, the results are different as the relationship between the variables is used.")
    selectInput("sel_microagg_v", choices=possvars_numericmethods(),
      label=h5(translate("Select variables for microaggregation"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      selected=input$sel_microagg_v, width="100%", multiple=TRUE)
  })
  output$ui_microagg_agglevel <- renderUI({
    txt_tooltip <- translate("Specifies the group size")
    sliderInput("sl_microagg_aggr",
      label=h5(translate("Aggregation-level"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      min=1, max=15, step=1, value=3, width="100%")
  })
  output$ui_microagg_aggmeasures <- renderUI({
    ...
    radioButtons("sl_microagg_measure",
      label=h5(translate("Aggregation statistics"), tipify(icon("question"), title=txt_tooltip, placement="top")),
  ...
  output$ui_microagg_trim <- renderUI({
    txt_tooltip <- translate("Sets the trimming percentage if the aggregation statistic is trim. For other methods, this is ignored.")
    sliderInput("sl_microagg_trim",
      label=h5(translate("Trimming-percentage"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      min=0, max=0.5, step=0.01, value=0, width="100%")
  })
  output$ui_microagg_varsort <- renderUI({
    txt_tooltip <- translate("Select sorting variable for method ‘simple’ for sorting the records.")
    sel_varsort <- selectInput("sel_microagg_varsort",
      label=h5(translate("Select variable for sorting"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      choices=get_allNumericVars_name(), selected=input$sel_microagg_varsort, width="100%")
  })

  out <- fluidRow(
    column(12, h4(translate("Microaggregation for numerical variables"), align="center")),
    column(12, p(translate("Many different algorithms to microaggregate numeric key variables can be applied here. The most important
      parameter is the"),code("aggregation level"), translate("because it specifies how many observations are grouped together before replacing actual values with some kind of aggregate."), align="center")))
  ...
  btn_microagg <- myActionButton("btn_microagg", label=translate("Perform Microaggregation"), "primary")
...
output$ui_noise <- renderUI({
  ...
  if (!has_numkeyvars()) {
    return(fluidRow(
      column(12, h4(translate("The current sdcProblem contains no",code("numerical key variables"))), align="center"),
      column(12, p(translate("However,"),code("addNoise()"),translate("can only be applied on such variables!
  ...
  output$ui_noise_slider <- renderUI({
    ...
    if (input$sel_noise_method=="correlated2") {
      txt_tooltip <- translate("Delta is the parameter that determines the level of noise. The larger delta, the higher the noise level. Please refer to the literature for a more detailed description of the effect of delta.")
      lab <- h5(translate("Amount of noise (parameter 'delta')"), tipify(icon("question"), title=txt_tooltip, placement="top"))
      par <- c(value=0.1, min=0.1, max=2, step=0.01)
    } else if (input$sel_noise_method=="ROMM") {
      txt_tooltip <- translate("Parameter 'p' is the multiplication factor for the method ROMM and corresponds to the magnitude of the perturbation. The value zero leads to no changes and the default value 0.001 to virtually no changes.")
      lab <- h5(translate("Amount of noise (parameter 'p')"), tipify(icon("question"), title=txt_tooltip, placement="top"))
      par <- c(value=0.001, min=0.001, max=0.3, step=0.001)
    } else if (input$sel_noise_method=="correlated"){
      txt_tooltip <- translate("The added noise is proportional to the variance in the data. The specified amount of noise is the multiplier for the covariance matrix of the noise. For example, for the default value 150, the covariance matrix of the noise is 1.5 times the covariance matrix of the data. The added noise is generated from a multivariate normal distribution.")
      lab <- h5(translate("Amount of noise (parameter 'noise')"), tipify(icon("question"), title=txt_tooltip, placement="top"))
      ...
      txt_tooltip <- translate("The added noise is proportional to the variance in the data. The specified amount of noise is the multiplier for the standard deviation of the noise. For example, for the default value 150, the standard deviation of the noise is 1.5 times the standard deviation in the data.")
      lab <- h5(translate("Amount of noise (parameter 'noise')"), tipify(icon("question"), title=txt_tooltip, placement="top"))
      par <- c(value=150, min=0, max=300, step=1)
    } else {
  ...
  output$ui_noise_method <- renderUI({
    ...
    selectInput("sel_noise_method",
      label=h5(translate("Select the algorithm"), tipify(icon("question"), title=txt_tooltip, placement="bottom")),
  ...
  output$ui_noise_vars <- renderUI({
    ...
    selectInput("sel_noise_v", choices=get_numVars_names(),
      label=h5(translate("Select variables"), tipify(icon("question"), title=txt_tooltip, placement="bottom")),
  ...
  output$ui_noise_btn <- renderUI({
    ...
    if (has_numkeyvars() | length(input$sel_noise_v)>0) {
      btn <- myActionButton("btn_noise", label=translate("Add noise"), "primary")
    }
    btn
  })

  out <- fluidRow(
    column(12, h4(translate("Adding stochastic noise"), align="center")),
    column(12, p(translate("Here you can use various methods to add noise in order to perturb continuous variables. Note: stochastic noise is a probabilistic method and the results differ depending on the current seed for the random number generator."), align="center")))
...
output$ui_rankswap <- renderUI({
  if (!has_numkeyvars()) {
    return(fluidRow(
      column(12, h4(translate("The current sdcProblem contains no"),code(translate("numerical key variables"))), align="center"),
      column(12, p(translate("However,"),code("rankSwap()"),translate("can only be applied on such variables!
        Please modify the"),code("sdcProblem"), align="center"))))
  }
  ...
  output$ui_rankswap_btn <- renderUI({
    ...
    if (has_numkeyvars() | length(input$sel_rankswap_v)>0) {
      btn <- myActionButton("btn_rankswap", label=translate("Apply rank swapping"), "primary")
    }
    btn
  })

  output$ui_rankswap_vars <- renderUI({
    txt_tooltip <- translate("Note that the results are different if rank swapping is applied to single variables or to groups of variables, since the covariance matrix is preserved.")
    selectInput("sel_rankswap_v",
      choices=possvars_numericmethods(), label=h5(translate("Select variables"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      width="100%", multiple=TRUE)
  })
  output$sl_rankswap_top <- renderUI({
    txt_tooltip <- translate("The highest values can be top coded before applying rank-swapping in order to protect outliers. This parameter specifies the number of values to be top coded as percentage of the sample size. By default no values are top coded.")
    sliderInput("sl_rankswap_top",
      label=h5(translate("Percentage of highest values that are grouped together before rank swapping"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      min=0, max=25, step=1, value=0, width="100%")
  })
  output$sl_rankswap_bot <- renderUI({
    txt_tooltip <- translate("The lowest values can be bottom coded before applying rank-swapping in order to protect outliers. This parameter specifies the number of values to be bottom coded as percentage of the sample size. By default no values are bottom coded.")
    sliderInput("sl_rankswap_bot",
      label=h5(translate("Percentage of lowest values that are grouped together before rank swapping"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      min=0, max=25, step=1, value=0, width="100%")

  })
  output$sl_rankswap_k0 <- renderUI({
    txt_tooltip <- translate("The algorithm keeps the change in the means of the variables before and after rank swapping within a range based on the subset-mean preservation factor K_0. ")
    txt_tooltip <- paste0(txt_tooltip, translate("The absolute difference between the variable mean before and swapping (abs(X_1 - X_2), where X_1 is the (subset) sample mean before swapping "))
    txt_tooltip <- paste0(txt_tooltip, translate("and X_2 is the (subset) sample mean after swapping) is kept smaller than or equal to 2 * K_0 * X_1 / sqrt(N_S), where N_S is the sample size of the subset under consideration. Therefore, larger values of K_0 allow larger deviations."))
    sliderInput("sl_rankswap_k0",
      label=h5(translate("Subset-mean preservation factor"), tipify(icon("question"), title=txt_tooltip, placement="bottom")),
      min=0, max=1, step=0.01, value=0, width="100%")
  })
  output$sl_rankswap_r0 <- renderUI({
    txt_tooltip <- translate("The algorithm preserves the correlation between variables within a certain range based on the specified multivariate preservation factor R_0, such that R_1/R_2 > R_0 where R_1 is the correlation coefficient of the two variables after swapping, ")
    txt_tooltip <- paste0(txt_tooltip, translate("and R_2 is the correlation coefficient of the two variables before swapping."))
    sliderInput("sl_rankswap_r0",
      label=h5(translate("Multivariate preservation factor"), tipify(icon("question"), title=txt_tooltip, placement="top")),
      min=0, max=1, step=0.01, value=0.95, width="100%")
  })
  output$sl_rankswap_p <- renderUI({
    txt_tooltip <- translate("This parameter (P) describes the size of the rank range as percentage of the total sample size. So two records are eligible for swapping if their ranks, i and j respectively, satisfy abs(i-j) < P*N/100, where N is the total sample size.")
    sliderInput("sl_rankswap_p",
      label=h5(translate("Rank range as percentage of total sample size."), tipify(icon("question"), title=txt_tooltip, placement="top")),
      min=0, max=100, step=1, value=0, width="100%")
  })

  out <- fluidRow(
    column(12, h4(translate("Rank Swapping")), align="center"),
    column(12, p(translate("This is a method to be used on numeric or ordinal variables. The idea is to",tags$i("swap"),"values within a range
