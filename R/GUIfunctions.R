...
#
# @keywords internal
# @param obj an object of class \code{\link{data.frame}} containing micro data# @param lab list with label information generated by extractLabels
# @param valLab an object of class \code{\link{data.frame}} containing micro data# @param lab list with label information generated by extractLabels
...
writeSafeFile <- function(obj, format, randomizeRecords, fileOut, ...) {
  ...
  if (format=="dtaold") {
    ...
    write_dta(data=dat, path=fileOut)
    writeDoFile(obj = obj, valLab = obj$stata_labs[[2]], fileName = "default", fileOut = paste0(substr(fileOut, 1, nchar(fileOut) - 4), ".txt")) 
  }
  if (format=="csv") {
    inp <- list(...)
