# Converts factor variables into class labelled for export with as_factor
#
# @keywords internal
# @param varName variable names (variable names cannot be changed during SDC process)
# @param facVar variable of class factor with levels the numneric values in string format
...
factorToLabelled <- function(varName, facVar, lab){
  ...
  temp <- suppressWarnings(unlist(lapply(facVar[,1], function(x){if(grepl("]", x)){as.numeric(substr(as.character(x), 2, regexpr("]", as.character(x)) - 1))}else{as.integer(as.character(x))}})))
  ...
  if(!is.null(labinfo)){
    ...
    # If no value labels, return the integer values
    return(temp) 
  }
}

#' Updates levels after recoding
#'
#' @keywords internal
...
#' @export
#' @docType methods
updateLevel <- function(obj, var, before, after, lab) {
  updateLevelX(obj, var, before, after, lab)
}
setGeneric("updateLevelX", function(obj, var, before, after, lab) {
  standardGeneric("updateLevelX")
})

setMethod(f="updateLevelX", signature=c("sdcMicroObj"), definition=function(obj, var, before, after, lab){
  # updateLevel <- function(obj, var, before, after, lab){
...
#' Updates label information after recoding
#'
#' @keywords internal
#' @param obj object of class sdcMicroObj-class
#' @param var variable name (variable names cannot be changed during SDC process)
...
#' @param after name of new level after recoding
#' @param lab list with label information generated by extractLabels
#' @return modified label list
#' @author Thijs Benschop
#' @export
#' @docType methods
updateLabel <- function(obj, var, before, after, lab) {
  updateLabelX(obj, var, before, after, lab)
}
setGeneric("updateLabelX", function(obj, var, before, after, lab) {
  standardGeneric("updateLabelX")
})

setMethod(f="updateLabelX", signature=c("sdcMicroObj"), definition=function(obj, var, before, after, lab){
  # updateLabel <- function(obj, var, before, after, lab){
  if (!class(obj)=="sdcMicroObj") {
    stop("invalid input in argument 'obj'\n")
  }
  ...
  if(all(regexpr("]", ll) != -1)){
    ...
    if(length(before) == 1){
      names(labinfo)[which(labinfo == as.numeric(substr(as.character(before[1]), 2, regexpr("]", as.character(before[1])) - 1)))] <- after[1]
      labinfo[which(labinfo == before[1])] <- numCode
    }else{
      names(labinfo)[which(labinfo == as.numeric(substr(as.character(before[1]), 2, regexpr("]", as.character(before[1])) - 1)))] <- after[1]
      labinfo[which(labinfo == as.numeric(substr(as.character(before[1]), 2, regexpr("]", as.character(before[1])) - 1)))] <- numCode
      labinfo <- labinfo[-which(labinfo == as.numeric(substr(as.character(before[-1]), 2, regexpr("]", as.character(before[-1])) - 1)))]
    }
...
# @param lab list with label information generated by extractLabels
# @fileName file name of exported Stata file
# @param fileOut (character) file to which output should be written# @return NULL
# @author Thijs Benschop
writeDoFile <- function(obj, lab, fileName, fileOut){
...
for(i in 1:length(valLab)){
  if(!is.null(valLab[[i]])){ 
    ...
    for(j in 1:length(valLab[[i]])){
      curval     <- valLab[[i]][j]
      curlabel   <- names(valLab[[i]])[j]
      if (j == 1){
        curstring  <- paste0("lab def ", curvarname, " ", curval, " \"",  curlabel, "\";")
      } else{
        curstring  <- paste0("lab def ", curvarname, " ", curval, " \"",  curlabel, "\", add;")
      }
      doFileText <- c(doFileText, curstring)
    }
    doFileText <- c(doFileText, paste0("lab val ", curvarname, " ", curvarname, ";"), "")
  }
}    

# Write file
writeLines(doFileText, fileOut)
}
